    ...
    for overload in op[2]:
      if overload.kind == nnkCommentStmt:
        ...
        for i, arg in params:
          if arg.kind == nnkCall:
            ...
            if pre == "dst":
              ...
              if suf == "16" and params[i-1].eqIdent"dst32":
        ...
        if genPointerVersion:
          ...
          assert procParams[^1][0].eqIdent"imm32"
        ...
        if genLiteralDispatch:
          ...
          result.add quote do:
            template `procName`*(a: var Assembler[X86], dst32: static RegX86_32, literal: static int) =
              a.`procName`(dst32, int32 literal)
